...
SDL_Texture* TextureManager::LoadTexture(const std::string& path, ImageType type /*= ImageType::PNG*/) {
  if (m_texture != nullptr) return m_texture;
...
void TextureManager::set_srcRect(const RectPosition& pos, const RectSize& size) {
  m_src.x = pos.GetX();
  m_src.y = pos.GetY();
...
void TextureManager::set_dstRect(const RectPosition& pos, const RectSize& size) {
  m_dst.x = pos.GetX();
  m_dst.y = pos.GetY();
